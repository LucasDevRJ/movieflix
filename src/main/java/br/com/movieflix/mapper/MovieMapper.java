...
public class MovieMapper {
    ...
    public static Movie toMovie(MovieRequest movieRequest) {
        ...
        List<Category> categories = movieRequest.categories().stream()
                .map(category -> Category.builder().id(category.getId()).build()).toList();

        List<Streaming> streamings = movieRequest.categories().stream()
                .map(streaming -> Streaming.builder().id(streaming.getId()).build()).toList();
